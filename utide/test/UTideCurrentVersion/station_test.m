...
time_JD = netcdf.getVar(ncid,netcdf.inqVarID(ncid,'time_JD'));
time_second = netcdf.getVar(ncid,netcdf.inqVarID(ncid,'time_second'));
time = double(time_JD) + 678942 + double(time_second)/(24.0*3600.0);
...
coef = ut_solv(time, double(ua), double(va), lat(1), 'auto','NoTrend','Rmin',0.95,'OLS','NoDiagn','LinCI');
